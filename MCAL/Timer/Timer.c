...
///static void(* timer0_pCallbackFunc1)(uint8*temp) = NULL; // we don't know the address so we must set it (NULL)
...
void __vector_11 (void)
{
	...
	/*else if(timer0_pCallbackFunc1!=NULL)
	{
		timer0_pCallbackFunc1();
	}*/
	else
	{
		/*do nothing*/
...
/*
uint8 timer0_setCallBack1(void(*callBackFunc1)(uint8*temp))
{
	uint8 local_error_state=OK;
	...
	{
		 timer0_pCallbackFunc1(&temp) = callBackFunc1(&temp);  // global pointer = local address of func to go to ISR
	}
	return local_error_state;
}*/